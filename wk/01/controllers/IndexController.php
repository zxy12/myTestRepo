...
class IndexController extends Cola_Controller
{
    public function indexAction()
    {
        ...
        //echo 'Index@', time();
        $username = mysql_escape_string($this->get('username'));
        $code     = mysql_escape_string($this->get('code'));
        $sid      = mysql_escape_string($this->get('key'));
        
        $model = $this->model('Data');
        $data = $model->find("sid='{$sid}' AND username='{$username}' AND code='{$code}'");
        if ($data && $data[0]) {
            if ($data[0]['num'] > 0) {
                echo $data[0]['code'];
